...
MERGE (fsys:Facultad {nombre:'ADMIN_SISTEMA', descripcion:'Facultad para la modificación de parámetros del sistema',
       tipo:"FACULTAD_SIMPLE",usuarioModificacion:'START', fechaModificacion:localdatetime(), activo: true})
MERGE (fadminACME:Facultad {nombre:'Admin_ACME', descripcion:'Facultad para la administración empresas ACME',
       tipo:"FACULTAD_SIMPLE",usuarioModificacion:'START', fechaModificacion:localdatetime(), activo: true})
...
MERGE (rAdminACME)-[:TIENE_FACULTAD]->(fadminACME)
...
DROP CONSTRAINT unique_usuario;
DROP CONSTRAINT unique_usuario2;

CREATE CONSTRAINT unique_compania FOR (compania:Compania) REQUIRE compania.nombre IS UNIQUE
CREATE CONSTRAINT unique_usuario FOR (usuario:Usuario) REQUIRE usuario.idUsuario IS UNIQUE
CREATE CONSTRAINT unique_usuario2 FOR (usuario:Usuario) REQUIRE usuario.nombreUsuario IS UNIQUE
